	<script  type="text/javascript" >
		$(function(){
			...
			getUrl<?=$objectId;?> = function (tipe){
				...
				<?}?>
				var filsasaran = $("#kode_sasaran_e1<?=$objectId;?>").val();
				...
				if (tipe==1){
					return "<?=base_url()?>pengukuran/rpt_pengukurane1/grid/"+filtahun+"/"+filsasaran+"/"+filiku+"/"+file1;
				}
				else if (tipe==2){
					return "<?=base_url()?>pengukuran/rpt_pengukurane1/pdf/"+filtahun+"/"+filsasaran+"/"+filiku+"/"+file1+paging;
				}else if (tipe==3){
					return "<?=base_url()?>pengukuran/rpt_pengukurane1/excel/"+filtahun+"/"+filsasaran+"/"+filiku+"/"+file1+paging;
				}
			...
			setTimeout(function(){
				// $('#dg<?=$objectId;?>').datagrid({
					// url:"<?=base_url()?>pengukuran/rpt_pengukurane1/grid",
					// queryParams:{lastNo:'0'},	
					// onLoadSuccess:function(data){
						// $('#dg<?=$objectId;?>').datagrid('options').queryParams.lastNo = data.lastNo;
						// var filtahun = $("#filter_tahun<?=$objectId;?>").val();
	  ...
	  <tr>
		<td>
		  <div class="fsearch" style="">
			...
			<tr>
				<td>Tahun :</td>
				<td><?=$this->rpt_pengukurane1_model->getListTahun($objectId)?></td>
			</tr>			
			<tr>
			...
			<tr>
				<td>Sasaran :</td>
				<td>	<span id="divSasaranE1<?=$objectId;?>">
	   ...
	   <div style="margin-bottom:5px">  
			<? if($this->sys_menu_model->cekAkses('PRINT;',272,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
				<a 	href="#" onclick="printData<?=$objectId;?>();" class="easyui-linkbutton" iconCls="icon-print" plain="true">Print</a>
			<?}?>
			<? if($this->sys_menu_model->cekAkses('EXCEL;',272,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
