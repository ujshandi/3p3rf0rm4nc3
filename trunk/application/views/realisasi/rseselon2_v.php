	...
	<script language="javascript">
		...
		function getDetail<?=$objectId;?>(){
			...
			$.ajax({ type: "GET",   
					 //url: base_url+'realisasi/rseselon2/getDetail/' + tahun + '/' + kode_kl + '/' + kode_sasaran_kl + '/<?=$objectId;?>',
					 url: base_url+'realisasi/rseselon2/getDetail/' + tahun + '/' + triwulan + '/' + kode_e2 + '/' + kode_sasaran_e2 + '/<?=$objectId;?>',
					 async: false,
					 success : function(text)
					 {
						 response = text;
			...
			//chan --------------------------------------
			if (response==""){
				$("#detail<?=$objectId;?>").html('Data PK tidak tersedia.');
			}
			else {
					...
					<form id="fm<?=$objectId;?>" method="post" style="margin:10px 5px 5px 10px;">		
						...
						<div class="fitem">
							...
							<span id="divSasaranE2<?=$objectId;?>">
								<?= "";//chan,$this->rseselon2_model->getListSasaranE2($objectId)?>
							</span>	
						</div>
						<br>
