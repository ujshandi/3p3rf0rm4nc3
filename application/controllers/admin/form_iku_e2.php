...
class Form_iku_e2 extends CI_Controller {
	...
	//	$userdata = array ('userLogin' => $userLogin,'logged_in' => TRUE,'groupId'=>$this->sys_login_model->groupId,'fullName'=>$this->sys_login_model->fullName,'userId'=>$this->sys_login_model->userId,'groupLevel'=>$this->sys_login_model->level);
		...
		$this->load->model('/admin/exception_e2_model');
	...
	function index(){
		$data['title'] = 'Formulasi IKK';		
		$data['objectId'] = 'formiku_e2';
	  	$this->load->view('admin/form_iku_e2_v',$data);
	...
	function grid($file1=null,$file2=null,$filapptype=null,$fillevel=null){
		...
		*/
		...
		echo $this->exception_e2_model->easyGrid($file1,$file2,$filapptype,$fillevel);
	}
	...
	public function getprefix($kode_e1,$kode_e2){
		$prefix ='';
		$prefix_iku ='';
		$data = $this->exception_e2_model->SelectInDb(($kode_e1=="-1"?"":$kode_e1),($kode_e2=="-1"?"":$kode_e2));
		//var_dump($data);
		if ($data!=null){
			$prefix = $data['prefix'];
	...
	function save($aksi="", $kode=""){
		...
		if ($this->form_validation->run() == FALSE){
			...
			if (!$this->exception_e2_model->isExistPrefix($data['prefix'],$data['prefix_old'])){
				$result = $this->exception_e2_model->saveToDb($data,$status);
