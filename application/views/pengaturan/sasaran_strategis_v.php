	<script  type="text/javascript" >
		$(function(){
			...
			newData<?=$objectId;?> = function (){  
				...
				$("#kode_sasaran_strategis<?=$objectId?>").removeAttr('readonly');
			}
			//end newData 
			...
			editData<?=$objectId;?> = function (editmode){
				...
				$("#kode_sasaran_strategis<?=$objectId?>").attr("readonly","readonly");
			...
			deleteData<?=$objectId;?> = function (){
				...
				if(row){
					if(confirm("Apakah yakin akan menghapus data '" + row.kode_sasaran_kl + "'?")){
						var response = '';
						$.ajax({ type: "GET",
								 url: base_url+'pengaturan/sasaran_strategis/delete/' + row.tahun + '/' + row.kode_sasaran_kl,
								 async: false,
			...
			getUrl<?=$objectId;?> = function (tipe){
				...
				if (tipe==1){
					...
					return "<?=base_url()?>pengaturan/sasaran_strategis/pdf/"+filtahun+"/"+filkey;
				}else if (tipe==3){
					return "<?=base_url()?>pengaturan/sasaran_strategis/excel/"+filtahun+"/"+filkey;
				}
			}
			//end getUrl
