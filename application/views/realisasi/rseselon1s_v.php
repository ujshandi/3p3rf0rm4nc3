	<script  type="text/javascript" >
		$(function(){
			...
			newData<?=$objectId;?> = function (){  
				//$('#dlg<?=$objectId;?>').dialog('open').dialog('setTitle','New rseselon1');  
				...
				<? if ($this->session->userdata('unit_kerja_e1')=='-1'){?>				
					...
					if(row){
						if(confirm("Apakah yakin akan menghapus data '" + row.kode_iku_e1 + "'?")){
							var response = '';
							$.ajax({ type: "GET",
									 url: base_url+'realisasi/rseselon1/delete/' + row.id_kinerja_e1,
									 async: false,
	...
	<table border="0" cellpadding="1" cellspacing="1" width="100%"> 
	 <tr>
	  ...
	  <div style="margin-bottom:5px">  
		<? if($this->sys_menu_model->cekAkses('ADD;',152,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
			<a href="#" onclick="newData<?=$objectId;?>();" class="easyui-linkbutton" iconCls="icon-add" plain="true">Add</a>  
		<?}?>
		<!------------Edit View-->
		<? if($this->sys_menu_model->cekAkses('EDIT;',152,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
			<a href="#" onclick="editData<?=$objectId;?>(true);" class="easyui-linkbutton" iconCls="icon-edit" plain="true">Edit</a>
		<?}?>
		<? if($this->sys_menu_model->cekAkses('VIEW;',152,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
			<a href="#" onclick="editData<?=$objectId;?>(false);" class="easyui-linkbutton" iconCls="icon-view" plain="true">View</a>
		<?}?>
		<? if($this->sys_menu_model->cekAkses('DELETE;',152,$this->session->userdata('group_id'),$this->session->userdata('level_id'))){?>
	...
	<table id="dg<?=$objectId;?>" class="easyui-datagrid" style="height:auto;width:auto" title="Data Realisasi Kinerja Eselon I" toolbar="#tb<?=$objectId;?>" fitColumns="true" nowrap="false" singleSelect="true" rownumbers="true" pagination="true">
	  ...
	  <tr>
		...
		<th field="deskripsi_sasaran_e1" hidden="true">Des Sasaran</th>
		<th field="kode_iku_e1" sortable="true" width="50px">Kode IKU</th>
		<th field="deskripsi_iku_e1" hidden="true">Des IKU</th>
