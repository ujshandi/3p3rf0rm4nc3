	<script  type="text/javascript" >
		...
		$(function(){
			...
			cancel<?=$objectId;?>=function(){
				$('#tt').tabs('close', '<?=($editmode==TRUE?'Edit':'View')?> PK Kementerian');
			}
			
		 	saveDataEdit<?=$objectId;?>=function(){
				$('#fmedit<?=$objectId;?>').form('submit',{
					url: base_url+'penetapan/penetapankl/save_edit',
					onSubmit: function(){
					...
					success: function(result){
						...
						if (result.success){
							...
							$('#tt').tabs('close', 'Edit PK Kementerian');
	...
	<div id="cc<?=$objectId;?>" class="easyui-layout" fit="true">  
		...
		<div region="north" split="true" title="" style="height:450px;">
			<div class="easyui-layout" fit="true">  
				<!------------Edit View-->
				<div region="center" border="true" title="<?=($editMode=="true")?"Edit":"View"?> Data Penetapan Kinerja (PK) Kementerian">
					<form id="fmedit<?=$objectId;?>" method="post" style="margin:10px 5px 5px 10px;">		
						<input type="hidden" name="id_pk_kl" value="<?=$result->id_pk_kl?>">
						<div class="fitem">
						...
						<div class="fitem">
							<label style="width:150px">Indikator Kinerja Utama :</label>
							<span style="display:block;margin-left: 150px;"><?=$result->iku_kl?></span>
						</div>
						<div class="fitem">
