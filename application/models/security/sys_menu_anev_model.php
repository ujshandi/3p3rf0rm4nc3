...
class Sys_menu_anev_model extends CI_Model 
{    var $shorcutList;
	 var $gotoMenuList;
	/**
	* constructor 
	*/
	...
	public function loadMenu($app_type,$imported=null,$cekPolicy=false){
		$sql = "select m.* from anev_menu m where hide<>1 and menu_parent is null and app_types like '%$app_type%'"
...
/*
		$this->db->select('*');
		$this->db->from('anev_menu');
		$this->db->where_like('policy','%AUTOTAB;%',false);
*/
		$sql = "select anev_menu.* from anev_menu inner join tbl_group_access g on anev_menu.menu_id = g.menu_id where g.policy like '%AUTOTAB;%'  and hide<>1 and group_id ='$group_id' ";//like '%$app_type%'  ";
		$query = $this->db->query($sql);
