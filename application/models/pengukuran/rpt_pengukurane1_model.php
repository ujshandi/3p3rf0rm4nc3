...
class Rpt_pengukurane1_model extends CI_Model
{	
	/**
	* constructor
	*/
	...
	public function easyGrid($filtahun=null,$filsasaran=null,$filiku=null,$file1=null,$purpose=1,$pageNumber=null,$pageSize=null){
		...
		if ($count>0){
			...
			if($filsasaran != '' && $filsasaran != '-1' && $filsasaran != '0' && $filsasaran != null) {
					$where1.=" and pk.kode_sasaran_e1='$filsasaran'";
					$where2.=" and pk.kode_sasaran_e1='$filsasaran'";
			}
			if($filiku != '' && $filiku != '-1' && $filiku != null) {
					$where1.=" and iku.kode_iku_e1='$filiku'";
					$where1.=" and iku.kode_iku_e1='$filiku'";
			}
...
from tbl_iku_eselon1 iku   inner join tbl_pk_eselon1 pk on pk.kode_iku_e1=iku.kode_iku_e1  '.$where1.'
			...
			foreach ($query->result() as $row){
				...
				$targetNow = $this->getTarget($filtahun,$row->kode_iku_e1);	
				$targetPast = $this->getTarget($filtahun-1,$row->kode_iku_e1);	
				$realisasiNow = $this->getRealisasi($filtahun,$row->kode_iku_e1);	
				$realisasiPast = $this->getRealisasi($filtahun-1,$row->kode_iku_e1);	
				...
				$response->rows[$i]['opini']=$this->getOpini($filtahun,$row->kode_iku_e1);
				$keterangan ='';
	...
	if ($purpose==1) //grid normal
			...
			to_excel($query,"PengukuranKinerjaE1",$colHeaders);
		}
